"""
calibration / hand_eye_view.py
================================
Hand-eye (eye-in-hand) 시각화: ``flange_to_cam`` + TCP pose → base 좌표계.

변환 체인::

    p_base = T_flange_to_base @ inv(T_flange_to_cam) @ p_cam

Usage::

  python examples/calibration/hand_eye_view.py
  python examples/calibration/hand_eye_view.py --save output/hand_eye.rrd
"""

from __future__ import annotations

import argparse
import logging
from pathlib import Path

import numpy as np

from rigid_transform_kit import Frame, RigidTransform
from rigid_transform_kit.viz import TransformVisualizer

from _viz_common import (
    add_common_args,
    finalize_viewer,
    load_4x4_matrices,
    load_and_preprocess_ply,
    log_camera_frustum,
    log_debug_link,
    log_origin_spheres,
    parse_tcp_vec6_from_filename,
)

log = logging.getLogger(__name__)

REPO_ROOT = Path(__file__).resolve().parent.parent.parent
_DEFAULT_DATA_DIR = REPO_ROOT / "datasets/hand_eye_example"
_DEFAULT_PLY = next(_DEFAULT_DATA_DIR.glob("*.ply"), _DEFAULT_DATA_DIR / "scan.ply")
_DEFAULT_CAL = _DEFAULT_DATA_DIR / "hand_eye_cal.yml"

_CAL_KEYS = ("flange_to_cam", "world_to_base")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Hand-eye viewer: PLY + flange_to_cam → robot base frame (Rerun).",
    )
    p.add_argument("--ply", type=Path, default=_DEFAULT_PLY, help="PLY (camera frame).")
    p.add_argument("--calibration", type=Path, default=_DEFAULT_CAL, help="YAML with flange_to_cam.")
    add_common_args(p)
    p.set_defaults(depth_max_m=1.5)
    return p.parse_args()


def main() -> None:
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
    args = parse_args()

    # ── Calibration ──
    cal = load_4x4_matrices(args.calibration, _CAL_KEYS)
    if "flange_to_cam" not in cal:
        raise KeyError(f"Need 'flange_to_cam': {args.calibration}")

    # flange_to_cam: p_cam = M @ p_flange → from=FLANGE, to=CAMERA
    T_flange_to_cam = RigidTransform.from_matrix(cal["flange_to_cam"], Frame.FLANGE, Frame.CAMERA)
    log.info("Loaded flange_to_cam from %s", args.calibration)

    # world_to_base: p_base = M @ p_world → from=WORLD, to=BASE
    T_world_to_base: RigidTransform | None = None
    if "world_to_base" in cal:
        T_world_to_base = RigidTransform.from_matrix(cal["world_to_base"], Frame.WORLD, Frame.BASE)
        log.info("Loaded world_to_base from %s", args.calibration)

    # ── TCP from filename ──
    vec6 = parse_tcp_vec6_from_filename(args.ply)
    if vec6 is None:
        raise SystemExit(
            f"Cannot parse TCP vec6 from: {args.ply.name}\n"
            "Expected: [idx_]x_y_z_W_P_R.ply"
        )
    # vec6 = TCP pose (base에서 본 flange) → p_base = M @ p_flange
    T_flange_to_base = RigidTransform.from_vec6(vec6, Frame.FLANGE, Frame.BASE)
    log.info("TCP: xyz=(%.1f,%.1f,%.1f) WPR=(%.1f,%.1f,%.1f)", *vec6)

    # ── Compose: cam → flange → base ──
    # p_base = T_flange_to_base @ inv(T_flange_to_cam) @ p_cam
    #        = T_flange_to_base @ T_cam_to_flange @ p_cam
    T_cam_to_base = RigidTransform.from_matrix(
        T_flange_to_base.matrix @ np.linalg.inv(T_flange_to_cam.matrix),
        Frame.CAMERA, Frame.BASE,
    )

    # ── PLY ──
    pts_cam_mm, colors = load_and_preprocess_ply(
        args.ply,
        no_depth_clip=args.no_depth_clip,
        depth_min_m=args.depth_min_m,
        depth_max_m=args.depth_max_m,
        depth_axis=args.depth_axis,
        max_points=args.max_points,
    )
    pts_base = T_cam_to_base.transform_points(pts_cam_mm)
    log.info("Transformed %d points to base frame.", len(pts_base))

    # ── Rerun ──
    save_path = args.save
    if save_path is None and len(pts_base) > 500_000:
        save_path = _DEFAULT_DATA_DIR / "hand_eye.rrd"
    spawn = save_path is None
    if save_path is not None:
        save_path.parent.mkdir(parents=True, exist_ok=True)

    vis = TransformVisualizer("hand_eye_view", spawn=spawn, port=args.port,
                              views=[("Hand-Eye (base frame)", "world")])

    vis.log_transform("world/base", RigidTransform.identity(Frame.BASE),
                      axis_length=50.0, label="BASE")
    vis.log_transform("world/flange", T_flange_to_base,
                      axis_length=50.0, label="FLANGE")
    vis.log_transform("world/camera", T_cam_to_base,
                      axis_length=50.0, label="CAMERA")
    log_camera_frustum("world/camera/frustum", image_plane_distance=90.0)

    if T_world_to_base is not None:
        vis.log_transform("world/world_frame", T_world_to_base,
                          axis_length=240.0, label="WORLD")

    # origin spheres
    origins = [
        ("world/_origins/base", np.zeros(3), (255, 80, 80), "BASE"),
        ("world/_origins/flange", T_flange_to_base.t, (80, 255, 80), "FLANGE"),
        ("world/_origins/camera", T_cam_to_base.t, (80, 120, 255), "CAMERA"),
    ]
    if T_world_to_base is not None:
        origins.append(("world/_origins/world", T_world_to_base.t, (255, 255, 80), "WORLD"))
    log_origin_spheres(origins)

    # point cloud
    if colors is None:
        colors = np.tile(np.array([[40, 180, 255]], dtype=np.uint8), (len(pts_base), 1))
    vis.log_points("world/pcd", pts_base, colors=colors, radii=1.2)
    log.info("Logged %d points.", len(pts_base))

    # debug links
    log_debug_link("world/_debug/base_to_flange", np.zeros(3), T_flange_to_base.t)
    log_debug_link("world/_debug/flange_to_cam", T_flange_to_base.t, T_cam_to_base.t,
                   color=(120, 200, 255, 120))

    finalize_viewer(save_path, spawn=spawn, app_name="Hand-Eye (base frame)")


if __name__ == "__main__":
    main()
